...
function getDateString(dt) {
  //Note, theres an in-built js function String(5).padStart(2,'0'); to attach leading zero to single numbers//////
  return (String(dt.getMonth()+01).padStart(2,'0')) + '/' + String(dt.getDate()).padStart(2,'0') + '/' + dt.getFullYear();
}
...
function chooseDate(e) {
 ...
 // $(this).click(function(){alert("kkdkdk");});
// getElementById("dateEnd").value(targ.getAttribute('date'));
}
...
onDOMReady(function(){
  ...
  for (i=0; i<allElements.length; i++) {
    ...
    if (className=='datepicker' || className.indexOf('datepicker ') != -1 || className.indexOf(' datepicker') != -1) {
      ...
      img.src='./images/reports_image/icon/cal-timer.png'; 
      img.title='Show calendar';
      img.style='height:30px;width:25px';
      a.appendChild(img);
